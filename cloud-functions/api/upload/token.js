...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (!config.GITHUB_OWNER || !config.GITHUB_REPO) return error("REPO_NOT_CONFIGURED", "未配置目标仓库（GITHUB_OWNER / GITHUB_REPO）", 500);
    const token = await installationToken(env);
    return json({ token, owner: config.GITHUB_OWNER, repo: config.GITHUB_REPO, daily_remaining: reservation.remaining });
