...
const EDITABLE = new Set(["hero_background_url", "hero_blur", "accelerator_base_url", "daily_upload_limit", "max_file_mb", "partition_config", "allowed_users"]);
...
export async function onRequest({ request, env }) {
  try {
    ...
    if (!isAdminSession(session, env)) return error("FORBIDDEN", "只有管理员可以修改站点设置", 403);
    ...
    const normalizedEntries = entries.map(([key, value]) => {
      if (key === "accelerator_base_url") return [key, value.trim().replace(/\/$/, "")];
      if (key === "allowed_users") return [key, [...new Map(value.map((item) => { const trimmed = item.trim(); return [trimmed.toLowerCase(), trimmed]; })).values()]];
      if (LIMITS[key]) return [key, toNumber(value, LIMITS[key])];
      return [key, value];
    });
