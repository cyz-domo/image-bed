...
export async function onRequest({ request, env }) {
  try {
    ...
    const body = JSON.stringify(session ? { authenticated: true, login: session.login, avatar_url: `https://github.com/${encodeURIComponent(session.login)}.png?size=64` } : { authenticated: false });
