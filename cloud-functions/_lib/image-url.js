...
export function imageBase(env, settings = {}) {
  ...
  if (value) { try { const url = new URL(value); if (url.protocol === "https:" && !url.username && !url.password && !url.pathname.replace(/\/$/, "") && !url.search && !url.hash) return `${value.replace(/\/$/, "")}/gh/${env.GITHUB_OWNER}/${env.GITHUB_REPO}@main`; } catch {} }
