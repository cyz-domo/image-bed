...
export async function githubUser(code, redirectUri, env) { const config = runtimeEnv(env); const tokenResponse = await fetch("https://github.com/login/oauth/access_token", { method: "POST", headers: { Accept: "application/json", "content-type": "application/json" }, body: JSON.stringify({ client_id: config.GITHUB_APP_CLIENT_ID, client_secret: config.GITHUB_APP_CLIENT_SECRET, code, redirect_uri: redirectUri }) }); if (!tokenResponse.ok) throw new Error("GitHub OAuth 换取令牌失败"); const token = (await tokenResponse.json()).access_token; if (!token) throw new Error("GitHub OAuth 未返回令牌"); const headers = { Accept: "application/vnd.github+json", Authorization: `Bearer ${token}`, "X-GitHub-Api-Version": "2022-11-28" }; const userResponse = await fetch("https://api.github.com/user", { headers }); if (!userResponse.ok) throw new Error("GitHub 用户信息获取失败"); const user = await userResponse.json();
  // /user 的 email 通常是公开邮箱（多数人为空）；补充尝试读取主邮箱（需 App 开启 Email addresses 权限，失败则忽略）
  try { const emailsResponse = await fetch("https://api.github.com/user/emails", { headers }); if (emailsResponse.ok) { const emails = await emailsResponse.json(); if (Array.isArray(emails)) { const primary = emails.find((entry) => entry.primary && entry.verified) || emails[0]; if (primary?.email && !user.email) user.email = primary.email; } } } catch {}
  return user; }
