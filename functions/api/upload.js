...
const maxBytes = Number(process.env.MAX_FILE_SIZE || 10485760);
...
async function putFile(path, data, message) { const token = await installationToken(); const response = await fetch(`https://api.github.com/repos/${process.env.GITHUB_OWNER}/${process.env.GITHUB_REPO}/contents/${path}`, { method: "PUT", headers: { Accept: "application/vnd.github+json", Authorization: `Bearer ${token}`, "Content-Type": "application/json", "X-GitHub-Api-Version": "2022-11-28" }, body: JSON.stringify({ message, content: base64(new Uint8Array(data)), branch: "main" }) }); if (!response.ok) throw new Error(`GitHub 写入失败 (${response.status})`); }

export async function onRequest({ request }) {
  if (request.method !== "POST") return error("METHOD_NOT_ALLOWED", "只支持 POST", 405);
  const session = await readSession(request); if (!session) return error("UNAUTHENTICATED", "请先使用 GitHub 登录", 401);
  try {
    const form = await request.formData(); const file = form.get("file"); if (!file || typeof file.arrayBuffer !== "function") return error("FILE_REQUIRED", "请选择图片", 400); if (file.size > maxBytes) return error("FILE_TOO_LARGE", "图片不能超过 10 MB", 413);
    ...
    await putFile(path, output, `chore: upload ${path.split("/").pop()}`);
    const url = `https://cdn.jsdelivr.net/gh/${process.env.GITHUB_OWNER}/${process.env.GITHUB_REPO}@main/${path}`;
