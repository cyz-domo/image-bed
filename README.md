...
- **多用户**：管理员（`ALLOWED_GITHUB_LOGIN`）可在设置 → 用户里添加 GitHub 用户名或公开邮箱，名单内用户用 GitHub 登录即可上传；**数据全隔离**——每个用户只能看到、删除自己上传的内容，每日配额按用户独立计算，管理员可见并管理全部图片
- **大文件直传**：EdgeOne 函数请求体上限 6MB，因此视频一律、以及压缩后仍超过 5MB 的图片，由浏览器持 GitHub 安装令牌（约 1 小时有效，仅签发给已登录管理员）**直接 PUT 到 GitHub API**，函数只负责签发凭证、校验文件存在并登记历史；视频封面由浏览器截帧后同令牌直传。**所有文件（含视频）不得超过 20 MB**——jsDelivr 对单文件有 20MB 分发上限，超限文件虽能存入 GitHub 但链接无法访问
