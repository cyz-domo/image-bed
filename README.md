...
- **视频直传**：EdgeOne 函数请求体上限 6MB，因此视频由浏览器持 GitHub 安装令牌（约 1 小时有效，仅签发给已登录管理员）**直接 PUT 到 GitHub API**，函数只负责签发凭证、校验文件存在并登记历史；封面由浏览器截帧后同令牌直传。**所有文件（含视频）不得超过 20 MB**——jsDelivr 对单文件有 20MB 分发上限，超限文件虽能存入 GitHub 但链接无法访问
